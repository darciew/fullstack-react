...
function switchApp(filePath, client) {
  ...
  var newContents = data.replace(/^import App from .*/m, "import App from \"" + filePath + "\";")
...
function assertInitialState(client) {
  client.
    expect.element('div.ui.comments > div:nth-child(1)').text.to.match(/Twelve minutes to ignition/)
}

function createMessage(client, message) {
  client.
    setValue('input[type=text]', message)
...
function assertCanCreateMessagesNoInitialState(client) {
  ...
  client.
    expect.element('.comment').text.to.match(/message1/);
  ...
  client.
    expect.element('div.ui.comments > div:nth-child(1)').text.to.match(/message1/)
  client.
    expect.element('div.ui.comments > div:nth-child(2)').text.to.match(/message2/)
}
...
function assertCanCreateMessagesWithInitialState(client) {
  ...
  client.
    expect.element('div.ui.comments > div:nth-child(2)').text.to.match(/message1/);
  ...
  client.
    expect.element('div.ui.comments > div:nth-child(2)').text.to.match(/message1/)
  client.
    expect.element('div.ui.comments > div:nth-child(3)').text.to.match(/message2/)
}

function assertTabularMenu(client) {
  client.
    assert.elementPresent('.ui.top.attached.tabular.menu')
  client.
    expect.element('div.tabular.menu > div.active.item').text.to.equal('Buzz Aldrin')
  client.
    expect.element('div.tabular.menu > div:nth-child(2)').text.to.equal('Michael Collins')
}
...
function assertWorkingTabularMenu(client) {
  ...
  // switch to mike
  client.
    click('div.tabular.menu > div:nth-child(2)')

  client.
    expect.element('div.tabular.menu > div.active.item').text.to.equal('Michael Collins')

  // switch back
  client.
    click('div.tabular.menu > div:nth-child(1)')
...
function assertFullyFunctional(client, initialState = false) {
  ...
  // no messages here
  client.
    assert.elementNotPresent('div.ui.comments > div')

  client.
    setValue('input[type=text]', 'mike\-message1')
  ...
  // we added a message
  client.
    expect.element('.comment').text.to.match(/mike\-message1/);
  ...
  if (initialState) {
    ...
    client.
      assert.elementNotPresent('div.ui.comments > div:nth-child(3)')
    ...
    // we expect his message to still be here
    client.
      expect.element('.comment').text.to.match(/message2/);

    // we don't exepct any more than this message
    client.
      assert.elementNotPresent('div.ui.comments > div:nth-child(2)')
...
function assertCanCreateAndDeleteMessagesWithInitialState(client) {
  ...
  client.
    click('div.ui.comments > div:nth-child(2)')
  client.
    expect.element('div.ui.comments > div:nth-child(2)').text.to.match(/message2/)
}
...
function assertCanCreateAndDeleteMessagesNoInitialState(client) {
  ...
  client.
    expect.element('div.ui.comments > div:nth-child(1)').text.to.match(/message2/)
}
...
module.exports = {
  ...
  "./complete/App-2.js": function(client) {
    ...
    assertCanCreateAndDeleteMessagesNoInitialState(client)
    client.
      expect.element('div.ui.comments > div:nth-child(1)').text.to.match(/@\d{13}/)
  },
